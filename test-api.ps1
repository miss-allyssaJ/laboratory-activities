...
try {
    ...
    Write-Host "[FAILED] Post creation failed: $($_.Exception.Message)" -ForegroundColor Red
    exit
}
...
try {
    ...
    Write-Host "[FAILED] Comment creation failed: $($_.Exception.Message)" -ForegroundColor Red
}
...
try {
    ...
    Write-Host "✗ Failed to retrieve comments: $($_.Exception.Message)" -ForegroundColor Red
}
...
try {
    ...
    Write-Host "[FAILED] Comment deletion failed: $($_.Exception.Message)" -ForegroundColor Red
}
...
try {
    ...
    if ($comments.Count -eq 2) {
        ...
        Write-Host "[FAILED] Expected 2 comments, found $($comments.Count)" -ForegroundColor Red
    }
} catch {
    Write-Host "✗ Failed to retrieve comments: $($_.Exception.Message)" -ForegroundColor Red
}
...
try {
    ...
    Write-Host "[FAILED] Post deletion failed: $($_.Exception.Message)" -ForegroundColor Red
}
